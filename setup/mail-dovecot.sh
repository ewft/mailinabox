...
tools/editconf.py /etc/dovecot/conf.d/10-ssl.conf \
	ssl=required \
	"ssl_cert=<$STORAGE_ROOT/ssl/mail.ewft.org.pem" \
	"ssl_key=<$STORAGE_ROOT/ssl/mail.ewft.org.pem" \
...
if [ ! -e "/etc/dovecot/dh.pem" ]
then
	echo "Generating DH parameters...."
	openssl dhparam -outform pem -out /etc/dovecot/dh.pem 4096 
fi
